...
for test in tests:
    ...
    test_parameters.append((port1.f_data, port1.z_data_raw, [results_list[3][1], results_list[0][1]], test))
print ''
...
def test_fit(f_data, z_data, fit_results, test):
    ...
    assert fit_results[0] == pytest.approx(results[4],abs=0.02*results[4]) and fit_results[1] == pytest.approx(results[5],0.02*results[5]), test